...
(function() {
  ...
  annotation_container.select(".power").style("left", force_g_offsetX).style("width", force_width).style("top", force_g_offsetY + force_height + 50);

  annotation_container.select(".pop").style("left", pack_g_offsetX).style("width", pack_width).style("top", pack_g_offsetY + pack_height + 50);

  control_container = d3.select(".control");

  control_container.style("left", force_g_offsetX).style("width", force_width + layout_spacing * 2 + pack_width).style("top", pack_g_offsetY + pack_height + 200);

  d3.json("data.json", function(err, data) {
    var active_force_label, active_force_node, active_pack_label, active_pack_node, cur_depth, cur_stage, first_run, force, force_data, force_labels, force_nodes, get_caption_text, hide_link, last_force_data, next, next_link, pack, pack_data, pack_nodes, pagination_li, pop_data, pop_label_links, pop_label_links_data, pop_labels, power_data, prev, prev_link, show_link, transitioning, update, update_active_links, update_control, update_force_node, update_mouse_binding, update_pack_node;
    ...
    get_caption_text = function(depth) {
      if (cur_stage === "stage1") {
        return "If you are " + "" + " under universal suffarage, you have the same voting power than anyone else";
      } else {
        return "If you are " + "" + ", you have at least " + "" + " voting power than in universal sufferage";
      }
    };
